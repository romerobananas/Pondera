...
export default class StepZilla extends Component {
  ...
  handleKeyDown(evt) {
    if (evt.which === 13) {
      if (
        !this.props.preventEnterSubmission &&
  ...
  jumpToStep(evt) {
    if (typeof evt.target === "undefined") {
      ...
      if (
        ...
        evt.target.value === this.state.compState
      ) {
        evt.preventDefault();
        evt.stopPropagation();
      ...
      this.abstractStepMoveAllowedToPromise(movingBack)
        ...
        .then(() => {
          ...
          if (proceed && !passThroughStepsNotValid) {
            if (
              evt.target.value === this.props.steps.length - 1 &&
  ...
  isStepAtIndexHOCValidationBased(stepIndex) {
    return (
      this.props.hocValidationAppliedTo.length > 0 &&
      this.props.hocValidationAppliedTo.indexOf(stepIndex) > -1
    );
  }
